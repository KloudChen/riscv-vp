...
ISS::ISS(sc_core::sc_module_name, uint64_t hart_id) :
...
uint64_t ISS::get_csr_value(uint64_t addr) {
	...
	auto read = [=](uint64_t reg, uint64_t mask) { return reg & mask; };
	...
	switch (addr) {
		...
		case MSTATUS_ADDR:
			return read(csrs.mstatus.reg, MSTATUS_READ_MASK);
		case SSTATUS_ADDR:
			return read(csrs.mstatus.reg, SSTATUS_READ_MASK);
		case USTATUS_ADDR:
...
void ISS::set_csr_value(uint64_t addr, uint64_t value) {
	auto write = [=](uint64_t reg, uint64_t mask) { reg = (reg & ~mask) | (value & mask); };
	...
	switch (addr) {
		...
		case MSTATUS_ADDR:
			write(csrs.mstatus.reg, MSTATUS_WRITE_MASK);
			break;
		case SSTATUS_ADDR:
			write(csrs.mstatus.reg, SSTATUS_WRITE_MASK);
			break;
		case USTATUS_ADDR:
